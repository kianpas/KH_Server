...
<section id=enroll-container>
	...
	<form id="memberUpdateFrm" method="post">
		<table>
			...
			<tr>
				<th>패스워드확인</th>
				<td><input type="password" id="password2" value="" required><br>
				</td>
			</tr>
			 -->
			<tr>
			...
			<tr>
				...
				<td><input type="radio" name="gender" id="gender0" value="M"
					...
					id="gender1" value="F" <%="F".equals(gender) ? "checked" : ""%>>
					<label for="gender1">여</label></td>
			</tr>
			<tr>
				<th>취미</th>
				<td><input type="checkbox" name="hobby" id="hobby0" value="운동"
					<%=hobbyChecked(hobbyList, "운동")%>><label for="hobby0">운동</label>
					<input type="checkbox" name="hobby" id="hobby1" value="등산"
					<%=hobbyChecked(hobbyList, "등산")%>><label for="hobby1">등산</label>
					<input type="checkbox" name="hobby" id="hobby2" value="독서"
					<%=hobbyChecked(hobbyList, "독서")%>><label for="hobby2">독서</label><br />
					<input type="checkbox" name="hobby" id="hobby3" value="게임"
					<%=hobbyChecked(hobbyList, "게임")%>><label for="hobby3">게임</label>
					<input type="checkbox" name="hobby" id="hobby4" value="여행"
					<%=hobbyChecked(hobbyList, "여행")%>><label for="hobby4">여행</label><br /></td>
			</tr>
		</table>
		<input type="button" onclick="updateMember();" value="정보수정" /> <input
			type="button" onclick="updatePassword();" value="비밀번호변경" /><input
			type="button" onclick="deleteMember();" value="탈퇴" />
	</form>
</section>
<script>
...
function updateMember(){
	$upFrm = $("#memberUpdateFrm");
	$upFrm.attr("action", "<%=request.getContextPath()%>/member/memberUpdate").submit();
	
}

function deleteMember(){
	$delFrm = $("#memberUpdateFrm");
	$delFrm.attr("action", "<%=request.getContextPath()%>/member/memberDelete").submit();
	 //탈퇴학인 처리까지 해줘야 한다.
